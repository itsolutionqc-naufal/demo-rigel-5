                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-neutral-200 dark:divide-neutral-700">
	                        @forelse($transactions as $index => $transaction)
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    @php
                                        $isHostSubmit = $transaction->transaction_type === 'host_submit';
                                        $displayAmount = $isHostSubmit ? (float) $transaction->commission_amount : (float) $transaction->amount;
                                    @endphp
                                    ...
                                    @if($isHostSubmit)
                                        <div class="text-xs text-neutral-500 dark:text-neutral-400">
                                            Komisi Talent Hunter (mengikuti Min. Nominal layanan)
                                        </div>
                                    @elseif($transaction->commission_amount > 0)
                                        <div class="text-xs text-neutral-500 dark:text-neutral-400">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span id="tx-status-badge-{{ $transaction->id }}" data-transaction-id="{{ $transaction->id }}" class="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-lg border
	                                ...
	                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
	                                    <div class="flex items-center justify-center gap-2">
	                                        <button type="button"
	                                            ...
	                                            class="inline-flex items-center gap-1 rounded-md border border-blue-600 bg-blue-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-blue-700 dark:border-blue-500 dark:bg-blue-500 dark:hover:bg-blue-600"
	                                            title="Lihat Detail">
	                                        ...
	                                        @if(auth()->user()->isAdmin())
		                                        <button type="button"
		                                            ...
		                                            class="inline-flex items-center gap-1 rounded-md border border-emerald-600 bg-emerald-600 px-2.5 py-1 text-xs font-medium text-white hover:bg-emerald-700 dark:border-emerald-500 dark:bg-emerald-500 dark:hover:bg-emerald-600"
		                                            title="Ubah Status">
    ...
    <script>
        ...
        if (document.readyState === 'loading') {
	        ...
	        @if(auth()->user()->isAdmin())
                ...
                function getTransactionStatusUi(status) {
                    const map = {
                        pending: {
                            label: 'Menunggu',
                            className: 'px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-lg border border-amber-400/50 text-amber-600 bg-amber-400/10 dark:border-amber-500/50 dark:text-amber-400 dark:bg-amber-500/10',
                        },
                        process: {
                            label: 'Proses',
                            className: 'px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-lg border border-blue-400/50 text-blue-600 bg-blue-400/10 dark:border-blue-500/50 dark:text-blue-400 dark:bg-blue-500/10',
                        },
                        success: {
                            label: 'Sukses',
                            className: 'px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-lg border border-emerald-400/50 text-emerald-600 bg-emerald-400/10 dark:border-emerald-500/50 dark:text-emerald-400 dark:bg-emerald-500/10',
                        },
                        failed: {
                            label: 'Gagal',
                            className: 'px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-lg border border-red-400/50 text-red-600 bg-red-400/10 dark:border-red-500/50 dark:text-red-400 dark:bg-red-500/10',
                        },
                    };

                    return map[status] || {
                        label: status,
                        className: 'px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-lg border border-neutral-400/50 text-neutral-600 bg-neutral-400/10 dark:border-neutral-500/50 dark:text-neutral-300 dark:bg-neutral-500/10',
                    };
                }

                function updateTransactionStatusBadge(transactionId, newStatus) {
                    const badge = document.getElementById(`tx-status-badge-${transactionId}`);
                    if (!badge) return;

                    const ui = getTransactionStatusUi(newStatus);
		        ...
		        function handleSubmitStatus(event) {
		            ...
		            fetch(action, {
		                ...
		                .then(data => {
		                    ...
		                    if (data.message) {
                                const transactionId = Number((data.transaction_id ?? '').toString()) || Number(action.match(/\/(\d+)\/status$/)?.[1] || 0);
                                const newStatus = data.new_status || formData.get('status');
                                if (transactionId > 0) {
                                    updateTransactionStatusBadge(transactionId, newStatus);
                                }

                                if (data.fcm_sent) {
                                    showToast('success', `${data.message} Notifikasi Firebase terkirim.`);
                                } else if (data.fcm_error) {
                                    showToast('warning', `${data.message} Status diperbarui, tapi Firebase: ${data.fcm_error}`);
                                } else {
		                ...
		                .catch(error => {
		                    ...
		                    showToast('error', 'Terjadi kesalahan saat memperbarui status: ' + error.message);
		                });
		        }
	        @endif
	        ...
	        // Detail modal functions
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        // Format amount
                        const displayAmount = transaction.transaction_type === 'host_submit'
                            ? (transaction.commission_amount || 0)
                            : (transaction.amount || 0);
                        document.getElementById('detailAmount').textContent = 'Rp ' + displayAmount.toLocaleString('id-ID');
