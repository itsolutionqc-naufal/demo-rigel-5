<x-layouts::app :title="__('Edit Telegram Bot')">
    <div class="container mx-auto px-4 py-6 space-y-6">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-neutral-900 dark:text-white">Edit Telegram Bot</h1>
                <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">{{ $telegramBot->name }} ({{ '@' . $telegramBot->username }})</p>
            </div>
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-6">
            <form action="{{ route('admin.telegram-bots.update', $telegramBot) }}" method="POST" class="space-y-5">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Nama Bot</label>
                    <input type="text" name="name" value="{{ old('name', $telegramBot->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="username" value="{{ old('username', $telegramBot->username) }}" required
                        class="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-neutral-900 dark:focus:ring-neutral-500" />
                ...
                <div>
                    ...
                    <input type="text" name="token" value="{{ old('token', $telegramBot->token) }}" required autocomplete="off"
                        class="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-neutral-900 dark:focus:ring-neutral-500" />
                ...
                <div>
                    ...
                    <input type="text" name="chat_id" value="{{ old('chat_id', $telegramBot->chat_id) }}" required
                        class="w-full rounded-lg border border-neutral-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 px-3 py-2 text-sm text-neutral-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-neutral-900 dark:focus:ring-neutral-500" />
                    @error('chat_id')<p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>@enderror
                </div>

                <div class="flex items-center gap-3">
                    <input id="is_active" type="checkbox" name="is_active" value="1" {{ old('is_active', $telegramBot->is_active) ? 'checked' : '' }}
