<x-layouts::app :title="__('Tambah Layanan')">
    <div class="container mx-auto px-4 py-6">
        @php
            $isTalentHunter = ($category ?? request('category')) === 'talent_hunter';
            $selectedCategory = old('category', $category ?? request('category'));
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-bold text-neutral-900 dark:text-white">
                    {{ $isTalentHunter ? 'Tambah Aplikasi Talent Hunter' : 'Tambah Layanan Baru' }}
                </h1>
                <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">
                    {{ $isTalentHunter ? 'Buat aplikasi khusus Talent Hunter' : 'Buat layanan baru dalam sistem' }}
                </p>
            </div>
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 shadow-sm">
            <form method="POST" action="{{ route('services.store') }}" enctype="multipart/form-data">
                ...
                <div class="p-6 border-b border-neutral-200 dark:border-neutral-700">
                    ...
                    <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">
                        Lengkapi formulir di bawah untuk membuat layanan baru
                    </p>
                </div>
                ...
                <div class="p-6 space-y-6">
                    @if($isTalentHunter)
                        <input type="hidden" name="category" value="talent_hunter">
                        <div class="rounded-lg border border-indigo-200 bg-indigo-50 p-4 dark:border-indigo-800/60 dark:bg-indigo-900/20">
                            <div class="flex items-start gap-3">
                                <i data-lucide="info" class="size-5 text-indigo-600 dark:text-indigo-300"></i>
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-indigo-900 dark:text-indigo-100">Kategori: Talent Hunter</p>
                                    <p class="mt-1 text-xs text-indigo-700 dark:text-indigo-200/80">
                                        Layanan ini akan muncul di menu Hunter (mobile) dan bisa dipilih saat submit host.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <!-- Filename Display -->
                        <div id="filename-display" class="mt-2 text-sm text-neutral-600 dark:text-neutral-400 hidden">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                value="{{ old('commission_rate', $isTalentHunter ? '0' : '1.00') }}"
                                step="0.01"
                                min="0"
                                max="100"
                    ...
                    <div>
                        ...
                        <div class="flex rounded-lg shadow-sm">
                            ...
                            <input
                                ...
                                value="{{ old('whatsapp_number') ? (str_starts_with(old('whatsapp_number'), '62') ? substr(old('whatsapp_number'), 2) : old('whatsapp_number')) : '8' }}"
                    ...
                    <!-- Telegram Bot -->
                    <div>
	                        <flux:select name="telegram_bot_id" label="Bot Telegram" :value="old('telegram_bot_id')">
	                            <option value="">-- Gunakan Bot Default --</option>
	                            @foreach(\App\Models\TelegramBot::active()->get() as $bot)
	                                <option value="{{ $bot->id }}" {{ old('telegram_bot_id') == $bot->id ? 'selected' : '' }}>
                    ...
                    <div class="flex items-center justify-between rounded-lg border border-neutral-200 bg-neutral-50 p-4 dark:border-neutral-700 dark:bg-neutral-900/30">
                        ...
                        <label class="inline-flex items-center gap-2">
                            <input type="checkbox" name="is_active" value="1" class="size-4 accent-indigo-600" {{ old('is_active', '1') ? 'checked' : '' }}>
