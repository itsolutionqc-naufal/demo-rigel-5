        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex gap-2">
                @if(auth()->user()->isAdmin())
                    <a href="{{ route('users.deleted') }}" class="relative inline-flex items-center gap-2 px-4 py-2 bg-red-600 dark:bg-red-600 hover:bg-red-700 dark:hover:bg-red-700 text-white border border-red-700 dark:border-red-700 rounded-lg font-medium text-sm transition-colors">
                        <i data-lucide="history" class="size-4"></i>
                        <span>User Dihapus</span>
                        @php
                            $deletedCount = \App\Models\User::onlyTrashed()->count();
                        @endphp
                        @if($deletedCount > 0)
                            <span class="inline-flex items-center justify-center px-2 py-0.5 text-xs font-bold leading-none text-red-600 bg-white rounded-full min-w-[20px]">
                                {{ $deletedCount }}
                            </span>
                        @endif
                    </a>
                @endif
                <a href="{{ route($routePrefix.'users.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-600 text-neutral-900 dark:text-white border border-neutral-300 dark:border-neutral-600 rounded-lg font-medium text-sm transition-colors">
    ...
    >
        ...
        >
            ...
            <div class="p-6 space-y-2">
                ...
                <div class="mt-4 rounded-xl border border-emerald-200 dark:border-emerald-700 bg-emerald-50 dark:bg-emerald-900/20 p-3">
                    <p class="text-xs text-emerald-700 dark:text-emerald-300 font-medium mb-1">
                        ✓ Soft Delete - Data Aman
                    </p>
                    <p class="text-xs text-neutral-600 dark:text-neutral-400">
                        User tidak bisa login, tapi data transaksi tetap tersimpan. Akun bisa di-restore kapan saja dari menu "User Dihapus".
                    </p>
                </div>
            </div>
