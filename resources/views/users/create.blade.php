        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 shadow-sm">
            <form method="POST" action="{{ route($routePrefix.'users.store') }}" enctype="multipart/form-data">
                ...
                <div class="p-6 border-b border-neutral-200 dark:border-neutral-700">
                    ...
                    <p class="text-sm text-neutral-500 dark:text-neutral-400 mt-1">
                        Lengkapi formulir di bawah untuk membuat user baru
                    </p>
                </div>
                ...
                <div class="p-6 space-y-6">
                    ...
                    @if(auth()->user()->isAdmin())
                        <div>
                            <flux:select name="role" label="Role Pengguna" :value="old('role', 'user')" required>
                                <option value="user" {{ old('role', 'user') === 'user' ? 'selected' : '' }}>User</option>
                                <option value="admin" {{ old('role', 'user') === 'admin' ? 'selected' : '' }}>Admin</option>
                                <option value="marketing" {{ old('role', 'user') === 'marketing' ? 'selected' : '' }}>Marketing</option>
                            </flux:select>
                            @error('role')
                                <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <flux:select
                                ...
                                :value="old('marketing_owner_id')"
                            >
                                <option value="">— Tidak ada —</option>
                                @foreach(($marketingOwners ?? collect()) as $marketingOwner)
                                    <option value="{{ $marketingOwner->id }}" {{ (string) old('marketing_owner_id') === (string) $marketingOwner->id ? 'selected' : '' }}>
