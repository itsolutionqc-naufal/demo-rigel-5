<x-layouts::auth>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Check if running in Capacitor native platform (iOS/Android app)
            function isNativeApp() {
                // Method 1: Check Capacitor API (Recommended)
                if (window.Capacitor && typeof window.Capacitor.isNativePlatform === 'function' && window.Capacitor.isNativePlatform()) {
                    console.log('[download-prompt] Detected native app via Capacitor API');
                    return true;
                }
                
                // Method 2: Check Android WebView user agent
                const ua = window.navigator.userAgent;
                if (ua.includes('wv') || ua.includes('WebView')) {
                    console.log('[download-prompt] Detected native app via WebView user agent');
                    return true;
                }
            ...
            // Wait a bit for Capacitor to load, then check
            function checkAndShowModal() {
                // If running in native app, skip the modal entirely
                if (isNativeApp()) {
                    console.log('[download-prompt] Running in native app, skipping modal');
                    return;
                }

                console.log('[download-prompt] Running in web browser, checking if should show modal');
                ...
                try {
                    if (localStorage.getItem(lastClosedKey) === todayKey) {
                        console.log('[download-prompt] Modal already closed today, skipping');
                        return;
                    }
                } catch (e) {
                ...
                function open() {
                    console.log('[download-prompt] Opening modal');
                ...
                function close() {
                    console.log('[download-prompt] Closing modal');
            ...
            // Wait 100ms for Capacitor to be available, then check
