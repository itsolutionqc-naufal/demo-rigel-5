<x-layouts::app :title="__('Dashboard')">
                ...
                <table class="w-full">
                    ...
                    <tbody>
	                        @forelse($recentTransactions as $transaction)
                                ...
                                <td class="py-3 px-4">
                                    <span id="dashboard-status-badge-{{ $transaction->id }}" data-transaction-id="{{ $transaction->id }}" class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium border
    ...
    <script>
        ...
        function getRowStatusUi(status) {
            const map = {
                process: {
                    label: 'Sedang Proses',
                    className: 'inline-flex items-center px-2 py-1 rounded-md text-xs font-medium border border-amber-500/50 text-amber-600 dark:text-amber-400 bg-amber-500/10 dark:bg-amber-500/20',
                },
                success: {
                    label: 'Sukses',
                    className: 'inline-flex items-center px-2 py-1 rounded-md text-xs font-medium border border-emerald-500/50 text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 dark:bg-emerald-500/20',
                },
                failed: {
                    label: 'Gagal',
                    className: 'inline-flex items-center px-2 py-1 rounded-md text-xs font-medium border border-red-500/50 text-red-600 dark:text-red-400 bg-red-500/10 dark:bg-red-500/20',
                },
            };

            return map[status] || {
                label: status,
                className: 'inline-flex items-center px-2 py-1 rounded-md text-xs font-medium border border-neutral-500/50 text-neutral-700 dark:text-neutral-200 bg-neutral-500/10 dark:bg-neutral-500/20',
            };
        }
        ...
        function updateDashboardStatusBadge(transactionId, newStatus) {
            const badge = document.getElementById(`dashboard-status-badge-${transactionId}`);
            if (!badge) return;

            const ui = getRowStatusUi(newStatus);
        ...
        async function confirmStatusUpdate() {
            ...
            if (newStatus === currentStatus) {
                showToast('warning', 'Status baru tidak boleh sama dengan status saat ini!');
                return;
            }

            try {
                const response = await fetch(`/transactions/${currentTransactionId}/status`, {
                    method: 'POST',
                    headers: {
                ...
                if (!response.ok || !payload?.success) {
                    const errMsg = payload?.message || 'Terjadi kesalahan saat memperbarui status';
                    throw new Error(errMsg);
                }

                closeStatusUpdateModal();
                updateDashboardStatusBadge(currentTransactionId, payload.new_status || newStatus);
                currentStatus = payload.new_status || newStatus;

                if (payload.fcm_sent) {
                    showToast('success', `${payload.message} Notifikasi Firebase terkirim.`);
                } else if (payload.fcm_error) {
                    showToast('warning', `${payload.message} Status diperbarui, tapi Firebase: ${payload.fcm_error}`);
                } else {
                    showToast('success', payload.message);
                }
            } catch (error) {
                console.error('Error:', error);
                showToast('error', error.message || 'Terjadi kesalahan saat memperbarui status');
            }
        }
