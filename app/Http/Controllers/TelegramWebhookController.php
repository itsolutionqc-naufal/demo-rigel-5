...
class TelegramWebhookController extends Controller
{
    ...
    public function handleWebhook(Request $request)
    {
        ...
        // Only validate secret if webhook was set WITH a secret token.
        // When running via tunnel (local dev), webhook is set without secret,
        // so we skip validation if no secret header is present.
        if (! empty($secret)) {
            $incomingSecret = (string) $request->header('X-Telegram-Bot-Api-Secret-Token', '');
            // Only reject if a secret was sent but doesn't match (not if it's missing)
            if (! empty($incomingSecret) && ! hash_equals((string) $secret, $incomingSecret)) {
    ...
    protected function handleCallbackQuery(array $callbackQuery)
    {
        try {
            ...
            if (in_array($transaction->status, ['success', 'failed'], true)) {
                ...
                $statusLabel = $transaction->status === 'success' ? '✅ BERHASIL' : '❌ GAGAL';
                $title = $transaction->transaction_type === 'withdrawal'
                    ? ($transaction->status === 'success' ? '🟢 PENARIKAN BERHASIL' : '🔴 PENARIKAN GAGAL')
                    : ($transaction->status === 'success' ? '🟢 TRANSAKSI BERHASIL' : '🔴 TRANSAKSI GAGAL');
    ...
    protected function approveTransaction(SaleTransaction $transaction, string $callbackId, int $chatId, int $messageId, array $from, bool $messageHasCaption, TelegramService $telegram)
    {
        ...
        try {
            ...
            // Kirim FCM push notification ke user via job queue
            $notificationResult = app(\App\Services\NotificationService::class)
                ->notifyTransactionStatusChange($transaction, 'process', 'success');

            Log::info('Firebase notification jobs dispatched for approved transaction', [
                'transaction_code' => $transaction->transaction_code,
                'jobs_dispatched' => $notificationResult['jobs_dispatched'] ?? 0,
                'fcm_sent' => $notificationResult['fcm_sent'] ?? false,
            ]);
            ...
            if ($transaction->transaction_type === 'host_submit') {
                ...
                $confirmMessage = TelegramMessageFormatter::heading('🟢 PENARIKAN BERHASIL');
                $confirmMessage .= TelegramMessageFormatter::bullet('Kode', (string) $transaction->transaction_code);
                $confirmMessage .= TelegramMessageFormatter::bullet('Admin', (string) ($from['first_name'] ?? 'Admin'));
                $confirmMessage .= TelegramMessageFormatter::bullet('Waktu', now()->format('d/m/Y H:i:s'));
                $confirmMessage .= TelegramMessageFormatter::divider();
                ...
                $confirmMessage .= TelegramMessageFormatter::divider();
                $confirmMessage .= TelegramMessageFormatter::bullet('Status', '✅ BERHASIL', false);
            } else {
                $confirmMessage = TelegramMessageFormatter::heading('🟢 TRANSAKSI BERHASIL');
                ...
                $confirmMessage .= TelegramMessageFormatter::divider();
                $confirmMessage .= TelegramMessageFormatter::bullet('Status', '✅ BERHASIL');
            }
    ...
    protected function rejectTransaction(SaleTransaction $transaction, string $callbackId, int $chatId, int $messageId, array $from, bool $messageHasCaption, TelegramService $telegram)
    {
        ...
        try {
            ...
            // Kirim FCM push notification ke user via job queue
            $notificationResult = app(\App\Services\NotificationService::class)
                ->notifyTransactionStatusChange($transaction, 'process', 'failed');

            Log::info('Firebase notification jobs dispatched for rejected transaction', [
                'transaction_code' => $transaction->transaction_code,
                'jobs_dispatched' => $notificationResult['jobs_dispatched'] ?? 0,
                'fcm_sent' => $notificationResult['fcm_sent'] ?? false,
            ]);
            ...
            if ($transaction->transaction_type === 'host_submit') {
                ...
                $confirmMessage = TelegramMessageFormatter::heading('🔴 PENARIKAN GAGAL');
                $confirmMessage .= TelegramMessageFormatter::bullet('Kode', (string) $transaction->transaction_code);
                $confirmMessage .= TelegramMessageFormatter::bullet('Admin', (string) ($from['first_name'] ?? 'Admin'));
                $confirmMessage .= TelegramMessageFormatter::bullet('Waktu', now()->format('d/m/Y H:i:s'));
                $confirmMessage .= TelegramMessageFormatter::divider();
                ...
                $confirmMessage .= TelegramMessageFormatter::divider();
                $confirmMessage .= TelegramMessageFormatter::bullet('Status', '❌ GAGAL', false);
            } else {
                $confirmMessage = TelegramMessageFormatter::heading('🔴 TRANSAKSI GAGAL');
                $confirmMessage .= TelegramMessageFormatter::bullet('Kode Transaksi', (string) $transaction->transaction_code);
                $confirmMessage .= TelegramMessageFormatter::bullet('Admin', (string) ($from['first_name'] ?? 'Admin'));
                $confirmMessage .= TelegramMessageFormatter::bullet('Waktu', now()->format('d/m/Y H:i:s'));
                $confirmMessage .= TelegramMessageFormatter::divider();
                ...
                $confirmMessage .= TelegramMessageFormatter::divider();
                $confirmMessage .= TelegramMessageFormatter::bullet('Status', '❌ GAGAL');
            }
