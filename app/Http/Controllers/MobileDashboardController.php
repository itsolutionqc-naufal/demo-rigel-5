        ...
        if ($page === 'wallet') {
            ...
            $commissionHistory = $commissionQuery->limit(50)->get() // Limit to prevent memory issues
                ->map(function ($commission): array {
                    ...
                    $transactionAmount = $saleTransaction ? (float) $saleTransaction->amount : null;

                    // For host_submit, the "transaction amount" is intentionally 0; show commission as the main amount.
                    if ($saleTransaction && $saleTransaction->transaction_type === 'host_submit') {
                    ...
                    return [
                        ...
                        'transaction_amount' => $transactionAmount, // Keep transaction amount for display (when meaningful)
                        'status' => 'success',
            ...
            try {
                ...
                if (filled($payoutBotId)) {
                    ...
                    if (! $payoutTelegram) {
                        ...
                        // Added buttons for approval/rejection
                        $buttons = [
                            ['text' => '✅ BERHASIL', 'callback_data' => "approve|{$payoutBotId}|{$txCode}"],
                            ['text' => '❌ GAGAL', 'callback_data' => "reject|{$payoutBotId}|{$txCode}"],
                        ];

                        $tgResult = $payoutTelegram->sendMessageWithButtons($payoutTelegram->getChatId(), $message, $buttons);

                        // Store transaction code in cache for webhook callback handling
                        if (!empty($tgResult['success']) && isset($tgResult['message_id'])) {
                            cache()->set(
                                "tg_message_{$tgResult['message_id']}",
                                $txCode,
