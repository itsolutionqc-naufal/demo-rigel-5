...
class UserController extends Controller
{
    ...
    public function deleted(Request $request)
    {
        $actor = $request->user();
        if (! $actor || ! $actor->isAdmin()) {
            abort(403, 'Anda tidak memiliki akses untuk melihat user yang dihapus.');
        }
        ...
        $users = $query->withCount('saleTransactions')
            ->with(['saleTransactions' => function($q) {
                $q->selectRaw('user_id, SUM(amount) as total_amount')
                  ->groupBy('user_id');
            }])
            ->paginate(10)
    ...
    public function restore(Request $request, $id)
    {
        $actor = $request->user();
        if (! $actor || ! $actor->isAdmin()) {
            $message = 'Anda tidak memiliki akses untuk restore user.';
        ...
        if ($request->email !== $user->email) {
            if ($request->expectsJson()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Email tidak sesuai dengan email user yang akan di-restore.',
                ], 422);
            }

            return redirect()->route('users.deleted')->with('error', 'Email tidak sesuai dengan email user yang akan di-restore.');
        }
        ...
        return redirect()->route('users.deleted')->with('success', $message);
    }
    ...
    public function forceDestroy(Request $request, $id)
    {
        $actor = $request->user();
        if (! $actor || ! $actor->isAdmin()) {
            $message = 'Anda tidak memiliki akses untuk menghapus permanen user.';
