...
class TransactionController extends Controller
{
    ...
    public function update(Request $request, SaleTransaction $transaction)
    {
        ...
        if ($newStatus === $originalStatus) {
            ...
            return redirect()->back()->with('error', 'Status tidak berubah. Pilih status lain untuk mengirim notifikasi.');
        }
    ...
    public function updateStatus(Request $request, SaleTransaction $transaction)
    {
        ...
        $dispatchResult = NotificationService::takeLastDispatchResult((int) $transaction->id);
        if (! $dispatchResult) {
            $dispatchResult = app(NotificationService::class)->notifyTransactionStatusChange(
                $transaction->fresh(['user', 'admin']),
                $originalStatus,
                $newStatus
            );

            Log::warning('Notification fallback dispatch executed', [
