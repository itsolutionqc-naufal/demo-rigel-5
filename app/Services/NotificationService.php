...
class NotificationService
{
        ...
        if (($transaction->transaction_type ?? null) === 'withdrawal') {
            ...
            if ($user && isset($statusMessages[$newStatus])) {
                ...
                if (in_array($newStatus, ['success', 'failed'], true)) {
                    $pushResult = app(PushNotificationService::class)->sendToUser(
                        userId: (int) $user->id,
                    ...
                    $dispatchResult = $this->mergePushResult($dispatchResult, $pushResult);
                }
            }
        }

        if (in_array($newStatus, ['success', 'failed'], true)) {
            $adminPushResult = $this->notifyAdminsTransactionFinalStatus($transaction, $oldStatus, $newStatus);
            $dispatchResult = $this->mergePushResult($dispatchResult, $adminPushResult);
    ...
    protected function notifyAdminsTransactionFinalStatus($transaction, string $oldStatus, string $newStatus): array
    {
        $dispatchResult = [
            'fcm_sent' => false,
            'fcm_error' => null,
    ...
    protected function mergePushResult(array $base, array $pushResult): array
    {
        $pushSent = (bool) ($pushResult['fcm_sent'] ?? false);
        ...
        if (! $base['fcm_sent']) {
            $base['fcm_error'] = $this->formatPushErrors($pushErrors);
        }

        return $base;
    }

    protected function formatPushErrors(array $errors): ?string
    {
        $clean = array_values(array_filter(array_map(
            static fn ($error) => is_string($error) ? trim($error) : null,
            $errors
        )));
