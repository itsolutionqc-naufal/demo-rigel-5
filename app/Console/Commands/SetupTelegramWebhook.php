...
class SetupTelegramWebhook extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($targets as $target) {
            ...
            try {
                $response = Http::retry(2, 200)->post("https://api.telegram.org/bot{$token}/setWebhook", $payload);
            ...
            try {
                $infoResponse = Http::retry(2, 200)->get("https://api.telegram.org/bot{$token}/getWebhookInfo");
                ...
                $rows[] = [
                    $label,
                    'FAILED',
                    'Webhook set, but getWebhookInfo failed: ' . $e->getMessage(),
                    '',
                    '',
                ];
                continue;
